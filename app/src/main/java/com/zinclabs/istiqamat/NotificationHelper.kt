...
class NotificationHelper(private val context: Context) {
    ...
    fun rescheduleAllAlarms() {
        ...
        val jsonData = prefs.getString("appData", null) ?: return
        
        try {
            val jsonObject = org.json.JSONObject(jsonData)
            if (jsonObject.has("habits")) {
                val habits = jsonObject.getJSONArray("habits")
                for (i in 0 until habits.length()) {
                    val habit = habits.getJSONObject(i)
                    if (habit.has("time") && habit.getString("time").isNotEmpty()) {
                        val timeStr = habit.getString("time")
                        val parts = timeStr.split(":")
                        if (parts.size == 2) {
                            val hour = parts[0].toIntOrNull() ?: continue
                            val minute = parts[1].toIntOrNull() ?: continue
                            val id = habit.optLong("id", 0L)
                            val title = habit.optString("title", "Habit Reminder")
                            val intId = (id % Int.MAX_VALUE).toInt()
                            
                            val msg = "Time for $title! \u23F0"
                            scheduleHabitNotification(
                                intId, title, msg, hour, minute,
